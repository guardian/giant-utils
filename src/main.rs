...
async fn main() {
    ...
    match &cli.command {
        ...
        Commands::DeleteCollection {
            ...
            let result: Result<(), CliError> = (|| async {
                ...
                while !blobs.is_empty() {
                    for blob in blobs {
                        ...
                        println!("Deleted blob {} in {}", blob.uri, format_duration(delete_blob_start_time.elapsed().unwrap()));
                    }
                    blobs = client
                        .get_blobs_in_collection(collection, &ListBlobsFilter::All)
                        .await?;
            ...
            println!("Deleted some blobs in {}", format_duration(start_time.elapsed().unwrap()));
